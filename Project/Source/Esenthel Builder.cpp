...
{
   Text   t_vs_path, t_aac, t_physics;
   TextLine vs_path;
   CheckBox aac;
   ComboBox physics;
   ...
   void create()
   {
      Gui+=super::create(Rect_C(0, 0, 1.4, 0.29), "Options").hide(); button[2].show();
      Flt y=-0.05, h=0.06;
      ...
      T+=t_physics .create(Vec2(0.19, y),     "Physics Engine"); T+=physics .create(Rect_L(0.38, y, 1.0, 0.05), Physics_t, Elms(Physics_t)).func(OptionChanged, T).set(PHYS_ENGINE_BULLET, QUIET).disabled(true); y-=h;
      load();
      loadConfig();
...
static Bool EmbedAppResource(C Str &dest, File &src, UInt type)
{
   ...
   File f; if(f.appendTry(dest))
   {
      Long pos=f.pos();
      f.putUInt(CC4_CHNK);
      f.putUInt(4+src.left()); // type + src data
      f.putUInt(type);
      if(src.copy(f))
      {
         UInt skip=f.pos()-pos+4*2; // trailing skip + marker
         f.putUInt(skip);
         f.putUInt(CC4_CHNK);
         return f.flushOK();
      }
...
void EmbedEditorLinuxResources(C Str &dest)
{
   struct Resource {UInt type; CChar8 *path;};
   static C Resource resources[]=
   {
      {CC4('I', 'C', 'O', 'N'), "Assets/Icon.webp"},
      {CC4('P', 'A', 'K', 0),   "Assets/EngineEmbed.pak"},
      {CC4('P', 'A', 'K', 0),   "Assets/App.pak"},
   };
   FREPA(resources)
   {
      C Resource &res=resources[i];
      Str src=EditorSourcePath+res.path;
      if(FExistSystem(src) && !EmbedAppResource(dest, src, res.type))
      {
         Gui.msgBox("Error", S+"Can't embed Linux editor resource:\n"+src);
         return;
      }
   }
}
...
{
   Str src =EditorSourcePath+"Esenthel",
       dest=EditorPath      +"Esenthel";
   if(!FExistSystem(src))Gui.msgBox("Error", S+"Can't find Linux editor executable:\n"+src);else
   {
      Copy(src, dest);
      if(FExistSystem(dest))EmbedEditorLinuxResources(dest);else Gui.msgBox("Error", S+"Can't copy Linux editor from:\n"+src+"\nto:\n"+dest);
   }
}
...
TaskBase TaskBases[]=
{
   ...
   {"Compile Apple"             , "Compile the Engine in Release mode for Mac only"                                     , CompileEngineApple        , true , true},
   {"Compile Mac (Debug)"       , "Compile the Engine in Debug mode for Mac only"                                       , CompileEngineDebugMac     , false},
   {"Compile Apple (Debug)"     , "Compile the Engine in Debug mode for Mac only"                                       , CompileEngineDebugApple   , false, true},
#elif LINUX
